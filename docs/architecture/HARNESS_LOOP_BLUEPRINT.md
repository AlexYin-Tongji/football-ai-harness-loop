...
### 3.0 资料收集类四层 Pipeline

2026-07-02 起，日报、转会和预测的资料类入口先经过四个子 Harness，再进入具体写作或预测席位：
...
- URL 收集层只负责找全相关候选 URL/元数据/短摘录，不写结论。
- 资料精简层只负责准确压缩，删除重复噪声，不新增事实。
- 增强层根据精简资料提出补采需求，只能调用已登记只读工具；GIF/比赛动图当前只能转人工。
- 撰写层只接收精简证据包和增强素材，不重新研究，不扩展来源。

每层都有独立的最大轮次、最大重试、超时、成本预算和降级策略；跨层传递结构化对象，不传递完整 LLM 对话历史。
