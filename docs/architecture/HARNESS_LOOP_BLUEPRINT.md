...
### 3.0 资料收集类五层 Pipeline

2026-07-02 起，日报、转会和预测的资料类入口先经过资料子 Harness，再进入具体写作或预测席位。2026-07-03 追加 Leader 全局监督层，防止某个护栏、单一来源或单一人物把普通主题挤成偏题报告：

```text
Seed URL 资料收集层
  → Seed 资料精简层
  → Leader 初始分组
  → 专栏小组内循环（收集 → 精简 → 增强 → 覆盖检查，≤3轮）
  → Leader 复审
  → 撰写层覆盖收敛
  → 确定性质量门
```

- Seed URL/精简层只负责给 Leader 一个足够规划的当天快照，不作为最终完整资料池。
- Leader 初始分组从 `match_report`、`transfer_intel`、`coach_tactics`、`player_profile`、`off_field`、`context` 中选择小组，写明每组搜索轮次、证据种子、补采目标和媒体目标。
- 每个专栏小组内部都有 Scout、Refiner、Enhancer、Writer/Reviewer 四个角色，自己完成资料收集、精简、增强和覆盖检查；连续两轮无新增证据/媒体即停止。
- Leader 复审检查主题覆盖、关键人物护栏占比、栏目分布、媒体资产、上游降级和人工回退条件。普通主题若只剩单一关键人物材料，必须阻断而不是交给写作层硬写。
- 撰写层只接收 Leader 批准的栏目合同、精简证据包和增强素材；最终合稿若漏掉 Leader 栏目或把多场比赛/转会揉成一段，覆盖门会触发最多 3 次合稿收敛。

2026-07-03 起，URL 收集层加载 `source-discovery` 横向 Skill，资料精简层加载 `evidence-refinement` 横向 Skill。前者规定常见足球来源、原语言扩展和批准域名边界；后者规定每条摘要必须保留原链接、来源状态、关键数字和传闻标签。写作层改为编辑型报告：先组织变化、背景和意义，再展示证据边界，避免把正文写成可信度审计表。

同日起，赛事视觉增强改为证据驱动：增强层根据具体比赛/进球/VAR 证据生成官方视频目标，媒体层只在官方频道白名单中寻找可嵌入视频，并把缩略图注入 `report_cover`、`timeline` 或 `section`。未授权新闻图、社媒截图、GIF 和比赛片段不进入自动媒体资产。

每层都有独立的最大轮次、最大重试、超时、成本预算和降级策略；跨层传递结构化对象，不传递完整 LLM 对话历史。Leader 层优先使用模型做简单栏目规划，失败时退回确定性规划；后续替换为 Minimax/长上下文模型时仍必须输出相同 schema、检查点、阻断标志和人工回退建议。

每层还必须写入 `checkpoints`：输入规模、输出规模、模型轮次、工具轮次、候选 URL、正文片段、增强需求、Leader 决策、覆盖栏目和媒体资产数等摘要会进入最终 Harness Trace，便于检查哪一层交付不足或降级。

### 3.1 节点内 Tool Loop

仅 URL 资料收集层和增强层允许模型提出只读工具需求：
...
默认不使用无边界多智能体讨论。2026-07-03 起采用有界 Leader/Supervisor Agent + 预设专家小组：Leader 负责栏目规划和交付审查，不替专家席写稿；专家小组只处理分配给自己的栏目。若未来评估证明“独立来源审计员”显著降低错误，才把 evaluator 独立为另一个 Agent。
