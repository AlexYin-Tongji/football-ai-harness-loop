...
@mcp.tool()
async def search_bbc_football(
    query: str, report_type: str = "world_cup_daily", max_items: int = 12
) -> dict:
    """Search the approved BBC Sport Football RSS feed for recent evidence."""
    request = ConsumerReportRequest(
        report_type=ReportType(report_type),
        subject=query,
        report_date=date.today(),
    ...
    items = await collect_bbc_evidence(request, max_items=max_items)
    return {
...
@mcp.resource("source://bbc-football-rss/policy")
def bbc_source_policy() -> str:
    return (
        "Development/non-commercial discovery and citation. Store title, URL, "
        "time and short excerpt; attribute BBC and link the original article."
    )
